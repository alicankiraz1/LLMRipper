...
class TestLLMRipper(unittest.TestCase):
    ...
    def test_merge_and_save(self):
        ...
        output_dir = "./test_merged_model"
        self.ripper.load_model(self.test_model_name)
        self.ripper.load_tokenizer(self.test_model_name)
        self.ripper.merge_and_save(output_dir)
        self.assertTrue(Path(output_dir).exists())
        self.assertTrue(Path(output_dir, "config.json").exists())
        tokenizer_files = list(Path(output_dir).glob("tokenizer.*")) + list(Path(output_dir).glob("vocab.*")) + list(Path(output_dir).glob("merges.*"))
        self.assertTrue(len(tokenizer_files) > 0)
        model_files = list(Path(output_dir).glob("pytorch_model.*")) + list(Path(output_dir).glob("model.*"))
        self.assertTrue(len(model_files) > 0)

    def tearDown(self):
        """Test sonrası temizlik."""
        if Path("./test_merged_model").exists():
            import shutil
            shutil.rmtree("./test_merged_model")
